...
| 路径 | 干什么 |
|---|---|
| `/` | 主页：8 场景 + 必备柜 + 100 张卡浏览；右上 练习/数据 入口；右下 FAB 显示"今日 N 张待复习" |
...
public/
├── audio/                  # 200 个 Edge TTS mp3（4 scene × 25 卡 × 慢/正常）
├── favicon.svg
├── icon-{192,512,512-maskable,apple-touch}.png

scripts/
├── generate-tts.py         # Edge TTS 主流水线（skip-if-exists）
...
| 想改什么 | 改哪里 |
|---|---|
| 卡片内容 | `src/data/cards.json` （改 data、不改组件） |
...
uv run --with edge-tts python3 scripts/generate-tts.py
```
...
- 部署后首次打开 → service worker 预缓存全部 HTML / JS / CSS / 图标 / 字体 / 200 个 mp3（约 3.8 MB）
- 手机 Safari/Chrome：分享 → "添加到主屏幕"
- 主屏图标启动 → standalone 全屏
- 飞行模式可用：100 张卡 + 所有音频 + SRS 进度
